...
const MUTATION: &str = r#"
    mutation CreateTransactionSearchParams($input: CreateTransactionSearchParamsInput!) {
        createTransactionSearchParams(input: $input) {
            id
            name
            params
            createdAt
...
#[tokio::test]
async fn test_create_transaction_search_params_basic() -> Result<()> {
    ...
    let saved = data.get("createTransactionSearchParams").unwrap();
...
#[allow(clippy::unwrap_used)]
#[tokio::test]
async fn test_create_transaction_search_params_free_plan_insert_limit() -> Result<()> {
    let TestContext { schema, .. } = TestContext::new().await?;
    ...
    let response = response.into_result().map_err(|e| e[0].clone())?;
    let data = response.data.into_json()?;
    let saved_first = data.get("createTransactionSearchParams").unwrap();
...
#[allow(clippy::unwrap_used)]
#[tokio::test]
async fn test_create_transaction_search_params_premium_inserts() -> Result<()> {
    let TestContext { schema, .. } = TestContext::new().await?;
    ...
    let response = response.into_result().map_err(|e| e[0].clone())?;
    let data = response.data.into_json()?;
    let saved_second = data.get("createTransactionSearchParams").unwrap();
...
#[allow(clippy::unwrap_used)]
#[tokio::test]
async fn test_create_transaction_search_params_concurrent_inserts() -> Result<()> {
    ...
    // １件しかないことを確認する
    let saved_count = saved_transaction_search_params::Entity::find()
        .filter(
            Condition::all()
                .add(saved_transaction_search_params::Column::Auth0Id.eq("user1"))
