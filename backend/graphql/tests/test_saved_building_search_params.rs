...
async fn test_empty_result() -> Result<()> {
    ...
    assert_eq!(data["savedBuildingSearchParams"], json!([]));
    Ok(())
}

#[allow(clippy::unwrap_used)]
#[tokio::test]
async fn test_saved_building_search_params_basic() -> Result<()> {
    let TestContext { db, schema } = TestContext::new().await?;
    let now = Utc::now();
    // user1: 2件, user2: 1件, user1 deleted: 1件
    let _ = sql_entities::saved_building_search_params::ActiveModel {
    ...
    let data = response.data.into_json().unwrap();
    assert_eq!(
        data["savedBuildingSearchParams"].as_array().unwrap().len(),
        2
    );
...
#[allow(clippy::unwrap_used)]
#[tokio::test]
async fn test_saved_building_search_params_order() -> Result<()> {
    let TestContext { db, schema } = TestContext::new().await?;
    let now = Utc::now();
    ...
    assert!(response.is_ok(), "{:?}", response.errors);
    let data = response.data.into_json().unwrap();
    let params = data["savedBuildingSearchParams"].as_array().unwrap();
...
#[allow(clippy::unwrap_used)]
#[tokio::test]
async fn test_saved_building_search_params_attributes() -> Result<()> {
    let TestContext { db, schema } = TestContext::new().await?;
    ...
    let response = schema
        .execute(
            Request::new(QUERY)
                .data(Auth0Id("test_user".into()))
                .data(make_roles_j_reit_premium(true)),
        )
        .await;
    assert!(response.is_ok(), "{:?}", response.errors);
    let data = response.data.into_json().unwrap();
    let saved_params = &data["savedBuildingSearchParams"].as_array().unwrap()[0];
