...
const MUTATION: &str = r#"
    mutation UpdateTransactionSearchParams($input: UpdateTransactionSearchParamsInput!) {
        updateTransactionSearchParams(input: $input) {
            id
            name
            params
            createdAt
...
#[tokio::test]
async fn test_update_transaction_search_params_basic() -> Result<()> {
    ...
    let saved = data.get("updateTransactionSearchParams").unwrap();
...
#[allow(clippy::unwrap_used)]
#[tokio::test]
async fn test_update_transaction_search_params_edit_other_user() -> Result<()> {
    let TestContext { db, schema } = TestContext::new().await?;
    let now = Utc.with_ymd_and_hms(2024, 3, 15, 10, 30, 0).unwrap();
