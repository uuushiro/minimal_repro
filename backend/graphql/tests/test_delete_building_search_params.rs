...
const MUTATION: &str = r#"
    mutation DeleteBuildingSearchParams($input: DeleteBuildingSearchParamsInput!) {
        deleteBuildingSearchParams(input: $input) {
            id
            name
            params
            createdAt
...
#[tokio::test]
async fn test_delete_building_search_params() -> Result<()> {
    ...
    let saved = data.get("deleteBuildingSearchParams").unwrap();
    ...
    let count_result = saved_building_search_params::Entity::find()
        .filter(
            Condition::all()
                .add(saved_building_search_params::Column::Auth0Id.eq("user1".to_string()))
